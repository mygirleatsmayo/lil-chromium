...
- `{"type":"restore-focus","priorContext":{"kind":"external-app","pid":int?,"bundleId":string?}}` — extension asks the host to reactivate the external app a closing lil's prior context names. With a `pid` (recorded by the app at open time), the host first activates the eligible live process at that exact PID, then may fall back to an eligible live process with the same bundle ID. Without one (the user came back to the browser from outside it, which Chromium reports only as no focused window), the host resolves the app from its own activation history: the last regular app activated other than the host's browser, watched via `NSWorkspace` for the life of the host and seeded from the frontmost app at host start. If no history or no eligible process exists it does nothing; it never substitutes a browser window.
