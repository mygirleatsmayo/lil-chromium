...
- Validate with `swift test` in `mac/` (behavior tests for config, messages, routing, URL intent, and the palette; fixtures live in `mac/Tests/LilChromiumTests/Fixtures/`) and `make app` (release build). `make install` goes further: it replaces the live app in /Applications and rewrites every browser's native-host manifest.
